...
Page 80459 "Budget Consolidation Header"
{
        ...
        area(creation)
        {
            action("Suggest Consolidation Lines")
            {
                ...
                begin
                    ...
                    WorkplanCostElements_New.Reset;
                    WorkplanCostElements_New.SetRange("Workplan No.", Rec."Annual Workplan");
                    WorkplanCostElements_New.SetRange("Job No", Rec."Current Job");
                    if WorkplanCostElements_New.FindSet then begin
                        repeat
                            //clear Procurement Lines
                            ProcurementPlanLinesN.Reset;
                            ProcurementPlanLinesN.SetRange("Procurement Plan ID", WorkplanCostElements_New."Functional Procurment Plan No");
                            ...
                            ProcurementPlanEntryN.SetRange("Procurement Plan ID", WorkplanCostElements_New."Functional Procurment Plan No");
                            ...
                            DetailedPPPlanEntryN.SetRange("Functional Procurment Plan No", WorkplanCostElements_New."Functional Procurment Plan No");
                            DetailedPPPlanEntryN.SetRange("Workplan No.", Rec."Annual Workplan");
                            DetailedPPPlanEntryN.DeleteAll;

                        until WorkplanCostElements_New.Next = 0;
                    end;
                    //
                    ...
                    if WorkplanCostElements.FindSet then begin
                        repeat
                            ...
                            if (WorkplanCostElements."Plan Item No" <> '') and (WorkplanCostElements.Quantity <> 0) and (WorkplanCostElements."Unit Cost" <> 0) and (WorkplanCostElements.Amount <> 0) then begin
                                //CC//
                                Item.Reset;
                                Item.SetRange("No.", WorkplanCostElements."Plan Item No");
                                if Item.FindFirst then begin
                                    Item.TestField("Unit Cost");
                                    //              IF (Item."Last Direct Cost"<>0) THEN BEGIN
                                    //                IF Item."Unit Cost"<>Item."Last Direct Cost" THEN
                                    //                 // ERROR('Kindly advice the unit price to use, Item %1 Unit Cost %2 is not equal to Last Direct Cost %3',Item."No.",Item."Unit Cost",Item."Last Direct Cost");
                                    //                DirectCost:=Item."Last Direct Cost";
                                    //                END;
                                end;
                                //Procurement Lines
                                ...
                                //****Start Commented by Morris on 07th Aug 2023 to allow muitiple Activities into the procurement plan entry table
                                /* ProcurementPlanEntryOriginal.RESET;
                                 ...
                                 ProcurementPlanEntryOriginal.SETRANGE(Directorate,FnGetDirectorate("Annual Workplan",WorkplanCostElements."Activity Id"));
                                 ProcurementPlanEntryOriginal.SETRANGE(Department,FnGetDepartment("Annual Workplan",WorkplanCostElements."Activity Id"));

                                 IF NOT ProcurementPlanEntryOriginal.FINDSET THEN BEGIN*/

                                //****end Commented by Morris on 07th Aug 2023 to allow muitiple Activities into the procurement plan entry table
                                ...
                                ProcurementPlanEntry.Quantity := WorkplanCostElements.Quantity;
                                ProcurementPlanEntry."Unit Cost" := CalculateTotalAmount(WorkplanCostElements);
                                Item.Reset;
                                Item.SetRange("No.", WorkplanCostElements."Plan Item No");
                                if Item.FindFirst then begin
                                    if Item."Unit Cost" = CalculateTotalAmount(WorkplanCostElements) then begin
                                        ProcurementPlanEntry."Unit Cost" := Item."Unit Cost";
                                    end else
                                        if Item."Unit Cost" <> CalculateTotalAmount(WorkplanCostElements) then begin
                                            if (Item."Last Direct Cost" <> 0) then begin
                                                if Item."Unit Cost" <> Item."Last Direct Cost" then
                                                    ProcurementPlanEntry."Unit Cost" := Item."Last Direct Cost";
                                            end;
                                        end;
                                end;
                                // ProcurementPlanEntry."Unit Cost":=CalculateTotalAmount(WorkplanCostElements);
                                ...
                                //cc//
                                ProcurementPlanEntry."Q1 Quantity" := WorkplanCostElements."Q1 Quantity";
                                ProcurementPlanEntry."Q2 Quantity" := WorkplanCostElements."Q2 Quantity";
                                ProcurementPlanEntry."Q3 Quantity" := WorkplanCostElements."Q3 Quantity";
                                ProcurementPlanEntry."Q4 Quantity" := WorkplanCostElements."Q4 Quantity";
                                ProcurementPlanEntry."Q1 Amount" := WorkplanCostElements."Q1 Amount";
                                ProcurementPlanEntry."Q2 Amount" := WorkplanCostElements."Q2 Amount";
                                ProcurementPlanEntry."Q3 Amount" := WorkplanCostElements."Q3 Amount";
                                ProcurementPlanEntry."Q4 Amount" := WorkplanCostElements."Q4 Amount";
    ...
    var
        ...
        WorkplanCostElements_New: Record "Workplan Cost Elements";
        Item: Record Item;
        DirectCost: Decimal;
