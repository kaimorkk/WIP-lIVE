page 70154 "Request For Proposal List"
{
    ...
    SourceTableView = where("Procurement Method" = const(RFP), Status = filter(<> Released));
    UsageCategory = Lists;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field("Preliminary Evaluated"; "Preliminary Evaluated")
                {
                    ApplicationArea = all;
                    Editable = false;
                }
                field("Technical Evaluated"; "Technical Evaluated")
                {
                    ApplicationArea = all;
                }
                field("Financial Evaluated"; "Financial Evaluated")
                {
                    ApplicationArea = all;
                }
            }
