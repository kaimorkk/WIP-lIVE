report 52193555 "PR Payroll Summary By Grade"
{
    ApplicationArea = All;
    Caption = 'PR Payroll Summary By Grade';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/PR Payroll Summary By Grade.rdl';
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            dataitem("PR Period Transactions"; "PR Period Transactions")
            {
                ...
                DataItemTableView = where("Transaction Type" = filter(Income), "Transaction Code" = filter(<> 'TOT-DED' & <> 'TXCHRG' & <> 'TXBP' & <> 'PSNR' & <> 'PNSR' & <> 'PAYE' & <> 'NPAY' & <> 'NONTAX' & <> 'NHIF-RL' & <> 'NHIF' & <> 'INSR'));
