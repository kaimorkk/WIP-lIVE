Page 52194420 "Board Member Card"
{
    Caption = 'Vendor Card';
    PageType = Card;
    ...
    actions
    {
        area(navigation)
        {
            group(BoardMember)
            {
                ...
                action(EarningsDeductionsSetup)
                {
                    ApplicationArea = Basic;
                    Caption = 'Earnings/Deductions Setup';
                    RunObject = Page "Commissioner ED Setup";
                    RunPageLink = Commissioner = field("No.");
                }
                action(Allowances)
                {
                    ApplicationArea = Basic;
    ...
    begin
        "Vendor Posting Group" := 'BRD&TRIBUN';
    end;
